...
fn load_cfg(client_config: &String) -> Result<ClientConfig> {
    ...
    let ws_url =  "wss://jarrett-devnet-8fa6.devnet.rpcpool.com/283aba57-34a4-4500-ba4d-1832ff9ca64a";
    if ws_url.is_empty() {
        panic!("ws_url must not be empty");
    }
    let payer_path = "/home/ubuntu/7i.json";
    if payer_path.is_empty() {
        panic!("payer_path must not be empty");
    }
    let admin_path = "/home/ubuntu/7i.json";
    if admin_path.is_empty() {
        panic!("admin_path must not be empty");
    }

    let raydium_v3_program_str = "devi51mZmdwUJGU9hjN27vEz64Gps7uUefqxg27EAtH";
    ...
    let mut mint0 = None;
    let mint0_str = "BRjpCHtyQLNCo8gqRUr8jtdAj5AjPYQaoqbvcZiHok1k";
    ...
    Ok(ClientConfig {
        ...
        payer_path: payer_path.to_string(),
